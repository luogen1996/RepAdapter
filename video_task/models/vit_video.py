...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn_rep = RepAdapter(in_features=dim)
        self.mlp_rep = RepAdapter(in_features=dim)

    def forward(self, x):
        x = x + self.drop_path(self.attn(self.attn_rep(self.norm1(x))))
        residual = x
        x = self.act(self.fc1(self.mlp_rep(self.norm2(x))))
